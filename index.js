...
async function run() {
  try {
      ...
      sendEmail(
        {
          ...
          message:
            "please verify your account and enjoy more future",
        },
        user?.email
      );
      res.send(result);
    ...
    app.post("/cards", async (req, res) => {
      ...
      const data = {
        total_amount: orderForm?.totalPrice,
        currency: orderForm?.currency,
        tran_id: transactionId, // use unique tran_id for each api call
        success_url: `${process.env.SERVER_SIDE_URL}/payment/success?transactionId=${transactionId}`,
        fail_url: `${process.env.SERVER_SIDE_URL}/payment/fail?transactionId=${transactionId}`,
        cancel_url: `${process.env.SERVER_SIDE_URL}/payment/cancel`,
        ipn_url: `${process.env.SERVER_SIDE_URL}/payment/cancel`,
        shipping_method: "Courier",
        product_name: orderForm?.foodName,
        product_category: "Electronic",
        product_profile: orderForm?.productImage,
        product_quantity: orderForm?.quantity,
        cus_name: orderForm?.userName,
        cus_email: orderForm?.userEmail,
        cus_add1: orderForm?.address,
        cus_add2: "Dhaka",
        cus_city: "Dhaka",
        cus_state: "Dhaka",
        cus_postcode: "1000",
        cus_country: "Bangladesh",
        cus_phone: orderForm?.phoneNumber,
        cus_fax: "01711111111",
        ship_name: orderForm?.userName,
        ship_add1: "Dhaka",
        ship_add2: "Dhaka",
        ship_city: "Dhaka",
        ship_state: "Dhaka",
    ...
    app.post("/payment/success", async (req, res) => {
      ...
      if (!transactionId) {
        return res.redirect(
          `${process.env.CLIENT_SIDE_URL}/payment/fail`
        );
      }
      ...
      const userEmail = await cardsCollection.findOne({
        transactionId,
      });

      if (userEmail?.cus_email) {
        sendEmail(
          {
            subject: "Your order is success fully added.",
            message: "Please preview your order detail's",
          },
          userEmail?.cus_email
        );
      }
      ...
      if (result.modifiedCount > 0) {
        res.redirect(
          `${process.env.CLIENT_SIDE_URL}/payment/success?transactionId=${transactionId}`
        );
      }
    });
    ...
    app.get("/order-by-transaction-id/:id", async (req, res) => {
      const { id } = req.params;
      const order = await cardsCollection.findOne({
        transactionId: id,
      });
